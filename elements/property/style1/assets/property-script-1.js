class PropertyElementStyle1
{
    constructor(scope, $)
    {
        ...
        this.container = scope.find('.js_property_style_1_data');
        ...
        this.dataItem = { items: undefined, url: '' };
    ...
    normalizeOptions()
    {
        this.options.desktopNumberShow = parseInt(this.options.desktopNumberShow, 10);
        this.options.tabletNumberShow = parseInt(this.options.tabletNumberShow, 10);
        this.options.mobileNumberShow = parseInt(this.options.mobileNumberShow, 10);
    }
    ...
    renderLoading()
    {
        ...
        for (let i = 1; i <= this.options.desktopNumberShow; i++)
        {
            ...
            htmlString += `<div class="${hiddenMobile}${hiddenTablet}d-lg-block col-xs-${mobileNumberShow} col-sm-${tabletNumberShow} col-md-${numberShow} col-lg-${numberShow}">${loadingItem}</div>`;
        }
    ...
    loadData()
    {
        ...
        if (this.dataItem.items !== undefined)
        {
            propertyList.html(this.renderItem(this.dataItem.items));

            propertyLink.attr('href', this.dataItem.url);
        ...
        let data = {
            action: 'PropertyElementStyle1::loadProperty',
        ...
        request.post(ajax, data).then(function (response)
        {
            ...
            if (response.status === 'success')
            {
                ...
                self.dataItem.items = response.data.items;

                self.dataItem.url = response.data.slug;
...
$(window).on('elementor/frontend/init', function ()
{
    elementorFrontend.hooks.addAction(
        'frontend/ready/PropertyElementStyle1.default',
        function (scope, $)
        {
            const instance = new PropertyElementStyle1(scope, $);
